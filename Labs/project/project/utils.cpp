...
/**
* Quick select algorithm but returns index of kth smallest element.
*/
int quick_select_index(double* input, int p, int r, int k)
{
	...
	else if (k < length) return quick_select_index(input, p, j - 1, k);
	else  return quick_select_index(input, j + 1, r, k - length);
}